...
<header 
	class="fixed left-0 right-0 top-0 z-50 transition-all duration-300 ease-in-out {scrolled ? 'border-b border-gray-100 bg-white/90 backdrop-blur-md shadow-sm' : 'bg-transparent'}"
...
>
	<div class="container mx-auto px-6">
		<div class="flex h-16 items-center justify-between">
			<a 
				href="/" 
				class="group relative font-display text-lg font-semibold text-gray-900">
				<span class="relative z-10">LuminaPDF</span>
				<span class="absolute bottom-0 left-0 h-[5px] w-0 bg-primary/10 transition-all duration-300 ease-out group-hover:w-full"></span>
			</a>
			...
			<nav class="hidden items-center space-x-8 md:flex">
				{#each navItems as item}
					...
					>
						...
						<span class="absolute -bottom-0.5 left-0 h-[2px] w-0 bg-primary transition-all duration-300 ease-out group-hover:w-full"></span>
					</a>
				{/each}
			</nav>
			...
			<div class="flex items-center gap-2">
				<div class="hidden md:block">
					...
					>
						<span>Open App</span>
						<ArrowRight class="ml-1.5 h-3.5 w-3.5 opacity-50 transition-transform group-hover:translate-x-1 group-hover:opacity-100" />
					</Button>
				</div>
