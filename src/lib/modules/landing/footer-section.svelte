...
<footer id="footer-section" class="border-t border-gray-200 bg-gray-50 px-6 py-16 sm:py-20">
	...
	>
		<div class="grid gap-12 md:grid-cols-5 lg:gap-16">
			...
			<div class="md:col-span-2">
				...
				<p class="mb-8 max-w-md text-gray-600">
					Powerful PDF generation and screenshot API designed for developers. 
					Create pixel-perfect documents with a simple API call.
				</p>
			...
			{#each footerLinks as column, i}
				<div style="transition-delay: {i * 100}ms" class="transition-all duration-700 ease-out" class:opacity-100={sectionVisible} class:translate-y-0={sectionVisible} class:opacity-0={!sectionVisible} class:translate-y-4={!sectionVisible}>
					<h4 class="mb-5 font-display text-sm font-semibold uppercase tracking-wider text-gray-900">{column.title}</h4>
					<ul class="space-y-4">
						{#each column.links as link}
							<li>
								...
								>
									{link.name}
									{#if link.external}
										<ArrowUpRight size={14} class="ml-1 opacity-50 transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:opacity-100" />
									{/if}
								</a>
							</li>
						{/each}
		...
		<div class="mt-16 flex flex-col items-center justify-between border-t border-gray-200 pt-8 sm:flex-row">
			<p class="mb-4 text-sm text-gray-500 sm:mb-0">© {new Date().getFullYear()} LuminaPDF. All rights reserved.</p>
